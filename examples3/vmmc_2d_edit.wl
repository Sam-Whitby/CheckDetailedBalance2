...
(* ================================================================
   ...
   State:   flat array of length L^2 (row-major).
            State[[s]] = 0 (empty) or k \[Element] {1,...,N} (labeled particle).
   ...
   Move:    Choose a random occupied site as the seed particle. Propose
            ...
            direction d \[Element] {right, left, down, up}. Grow a cluster using
            the Whitelam\[Dash]Geissler VMMC link probabilities:

              wFwd = max(1 \[Minus] exp(e_init \[Minus] e_fwd),  0)   [forward  link weight]
              wRev = max(1 \[Minus] exp(e_init \[Minus] e_rev),  0)   [reverse  link weight]
            ...
            non-cluster neighbour q of cluster particle p:
              \[Bullet] with probability wFwd a link is attempted;
              \[Bullet] of those, fraction (1 \[Minus] wRev/wFwd) are frustrated links
                \[RightArrow] entire move is rejected;
              \[Bullet] fraction wRev/wFwd \[RightArrow] q joins the cluster.
...
(* All four nearest neighbours of site s on the L\[Times]L torus *)
$allNeighbors2D[s_, L_] := {$right2D[s,L], $left2D[s,L], $down2D[s,L], $up2D[s,L]}

(* Translate site s by direction vector {dRow, dCol} on the L\[Times]L torus *)
...
(* Nearest-neighbour only: couplingJ = 0 for d\.b2 \[NotEqual] 1 or if either site is a hole. *)
...
(* Unique undirected bonds on the L\[Times]L torus, memoised per L.
...
(* Virtual pair energy between a particle of type typeI at virtual site vI
   ...
   NN adjacency gives couplingJ at d\.b2=1; beyond NN gives 0. *)
...
(* Grow a VMMC cluster via BFS, starting from seed site with proposed
   rigid displacement dir = {dRow, dCol} on the L\[Times]L torus.
   ...
   Link logic (Whitelam-Geissler superdetailed balance):
     draw r1 ~ U[0,1]:
       if r1 > wFwd  \[RightArrow] no link (neighbour stays out of cluster)
       else draw r2 ~ U[0,1]:
         if r2 > wRev/wFwd \[RightArrow] frustrated link \[RightArrow] reject entire move
         else              \[RightArrow] q joins the cluster                      *)
      ...
      Do[
        ...
        If[state[[q]] =!= 0 && !KeyExistsQ[inCluster, q],
          ...
          (* Piecewise (not Max) so FullSimplify can resolve sign-cases of J.
             Leading {1, e===Infinity} guards prevent Exp[\[Beta](finite-Infinity)]
             with symbolic \[Beta], which would produce ComplexInfinity warnings.
             Ratio case guide: eFwd=\[Infinity],eRev=\[Infinity]\[RightArrow]1; eFwd=\[Infinity],eInit<eRev\[RightArrow]wRev;
             eFwd=\[Infinity]\[RightArrow]0; eRev=\[Infinity],eInit<eFwd\[RightArrow]1; else\[RightArrow]Min[wRev/wFwd,1]. *)
          wFwd = Piecewise[{
              {1,                               eFwd === Infinity},
...
Algorithm[state_List] :=
  Module[{L, occupied, seed, dirs, dir, cluster, newState, dE},
    ...
    dirs = {{0,1},{0,-1},{1,0}};   (* right, left, down *)
    ...
    (* Apply the rigid cluster translation:
       ...
       overlap that was not resolved during cluster growth \[RightArrow] reject.        *)
    newState = state;
...
(* Return all integer IDs that decode to L\[Times]L (perfect-square) arrays with
   ID \[LessEqual] maxId.  check.wls uses this to skip the ~99% of bit strings that
   BitsToState would immediately reject, reducing pre-3\[Times]3 overhead from
